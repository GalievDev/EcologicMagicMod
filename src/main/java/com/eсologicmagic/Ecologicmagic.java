...
public class Ecologicmagic {
    ...
    public Ecologicmagic() {
        ...
        EmaciatedBiome.register(eventBus);
    ...
    private void doClientStuff(final FMLClientSetupEvent event){
        event.enqueueWork(() -> {
           RenderTypeLookup.setRenderLayer(Block.byItem(Registration.ECOLOG_STAFF.get()), RenderType.cutout());
   ...
   private void setup(final FMLClientSetupEvent event) {
    ...
    public static void onRegisterItems(final RegistryEvent.Register<Item> event) {
        Registration.BLOCKS.getEntries().stream().map(RegistryObject::get).forEach(block -> {
            event.getRegistry()
                    .register(new BlockItem(block, new Item.Properties().tab(ECOLOGICALL))
